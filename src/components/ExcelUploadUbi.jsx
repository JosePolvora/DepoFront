...
function ExcelUploadUbi() {
    ...
    const handleFileUpload = async () => {
        ...
        try {
            ...
            const response = await axios.post('http://localhost:3000/api/upload-ubicaciones', formData, {
                headers: {
    ...
    return (
        <div className="bg-zinc-900 min-h-screen flex justify-center items-center p-6">
            <div className="bg-zinc-800 rounded-lg border-2 border-orange-500 p-8 shadow-md w-full max-w-3xl">
                <h1 className="text-2xl font-bold text-center text-orange-500 mb-6">Importar Ubicaciones</h1>
                    ...
                    <div className="mt-8 overflow-x-auto"> {/* Ajustado mt */}
                        ...
                        <table className="min-w-full bg-zinc-900 shadow-inner rounded-lg overflow-hidden border border-zinc-700">
                            <thead className="bg-zinc-700">
                                <tr>
                                    <th className="px-6 py-3 text-left text-xs font-semibold uppercase tracking-wider text-yellow-200">Ubicación</th>
                            ...
                            <tbody className="divide-y divide-zinc-700">
                                {uploadResult.result.map((item, index) => (
                                    ...
                                    >
                                        <td className="py-2 px-6 whitespace-nowrap text-sm text-white">{item.codigo}</td>
